        ...
        newButton.addEventListener("click", async () => {
               ...
               //This function helps to store your datas in the cart(located in the localStorage);
                ...
                async function storeDataInLocalStorage() {
                    ...
                    const dataExist = await myCart.find(data => data.id === dessertObject.id);
                     
                    //verifying of the data exist in the cart be fore adding a new one
                    if (!dataExist) {
            ...
            incrementIcon.addEventListener("click", async () => {
                ...
                let count = index.textContent = quantity;
                      ...
                      const dataExistInCart = await dataInCart.find(item=> item.id === dessertObject.id);
                
                      if(dataExistInCart){
                          dataExistInCart.quantity = quantity + 1;
                          await dataInCart.push(dataExistInCart);
                    ...
                    // console.log(addingToThisCart.info);
                ...
                addTOcart.append(cartItem, currentItem, dessertObject.price * count, 'hgh');
